...
struct sr_rt* sr_get_lpm_entry(struct sr_rt *routing_table, uint32_t ip){
  struct sr_rt *rt_ = NULL;
  unsigned long int longest_prefix = 0;
  while(routing_table != NULL){
    if(((unsigned long int)routing_table->mask.s_addr & (unsigned long int)ip) == (unsigned long int)routing_table->dest.s_addr){
